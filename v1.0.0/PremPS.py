...
def rf_contribution():
    ...
    os.system('rm %s' % (in_file+'.cleaned.outdata'))
    f = pd.read_csv(outdata+'.contribution', header=0, index_col=0,sep = '\t')
    f.drop(['bootstrapFC','PremPS.1'],axis=1).to_csv(out_file,sep='\t',index=0)
    os.system('rm %s' % (outdata+'.contribution'))
